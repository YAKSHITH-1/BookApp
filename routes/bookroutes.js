...
try {
    ...
    const book = new newBook(req.body).find().sort({createdAt:-1}).limit(limit).skip(skip).populate('user','username profileimage');

const totalbooks = await newBook.countDocuments();
